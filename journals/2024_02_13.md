	...
	- ## What you need to implement in D2:
		...
		- **What is output by the open function, created by the pipe function, and represented in `STDIN_FILENO` and `STDOUT_FILENO`?**
			...
			- Example fd table:
			  ...
			  | 4... | files you opened |
			- Implicitly, every process interprets whichever file is in index `0 == STDIN_FILENO` as stdin, in index 1 as stdout, and in index 2 as stderr. If we switch the file that is behind the in
			-
			-
			-
		- ```c
