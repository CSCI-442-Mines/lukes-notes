	...
	- ## What you need to implement in D2:
		...
		- **What is output by the open function, created by the pipe function, and represented in `STDIN_FILENO` and `STDOUT_FILENO`?**
			...
			- Example fd table:
			  ...
			  | 3... | files you opened |
			- Implicitly, every process interprets whichever file is in index `0 == STDIN_FILENO` as stdin, in index `1 == STDOUT_FILENO` as stdout, and in index `2 == STDERR_FILENO` as stderr. If we switch the file in index 1, and the process t
			-
			-
			-
		- ```c
