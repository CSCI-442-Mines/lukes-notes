	...
	- ## What you need to implement in D2:
		...
		- **How does pipe work?**
			- Lets refer to the manpage for pipe.
			  ...
			  The argument is an array of two integers. Recall that arrays are represented as a pointer to the first index, so by passing in an array, we are effectively passing by reference. What pipe will do is modify the array to contain the file descriptors (again, integers that index the file table) for the
			-